...
static uint8 ShakeStatusOld;
static uint8 ShakeStatusNow;
/*********************************************************************
 * LOCAL FUNCTIONS
 */
...
static void PhotoInit(void) //P00  AD   P01   开关
{
  ...
  if(SHAKE_GPIO == HIGH)
    ShakeStatusNow = SHAKE_NO;
  else
    ShakeStatusNow = SHAKE_YES;
}
/*********************************************************************
 * @fn      SerialApp_Init
 *
...
void SerialApp_Init( uint8 task_id )
{
  ...
  UART0_Format.Header   = '@';
  UART0_Format.Len      = 0x07;
  UART0_Format.NodeSeq  = 0x01;
  UART0_Format.NodeID   = Shake;
  UART0_Format.Verify   = 0x4b;
...
{ 
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SerialApp_TaskID )) )
    {
      ...
      case ZDO_STATE_CHANGE:
          ...
          if(SampleApp_NwkState == DEV_END_DEVICE) //判定当前设备类型
          {
            ...
            osal_set_event(SerialApp_TaskID, SHAKE_READ_EVT); //启动光照检测
          }
        break;
      default:
        break;
  ...
  if ( events & SHAKE_READ_EVT )  //查看光敏电阻状态
  {
    ShakeStatusOld = ShakeStatusNow;  //保存当前状态
    if(SHAKE_GPIO == HIGH)
    {
      ShakeStatusNow = SHAKE_NO;
    }
    else if(SHAKE_GPIO == LOW)
    {
      ShakeStatusNow = SHAKE_YES;
    }
    
    if(ShakeStatusNow != ShakeStatusOld) //如果当前状态和上一个状态发生变化，发送消息给网关
    {
      UART0_Format.Command = 0x01;
      if(ShakeStatusNow == SHAKE_YES)
      {
        UART0_Format.Data[0] = 0x01;
        UART0_Format.Data[1] = 0x00;
      }
      else if(ShakeStatusNow == SHAKE_NO)
      {
        UART0_Format.Data[0] = 0x00;
        UART0_Format.Data[1] = 0x00;       
      }
    ...
    osal_start_timerEx(SerialApp_TaskID, SHAKE_READ_EVT, 200);
    
    return ( events ^ SHAKE_READ_EVT );
  }
