...
{ 
  ...
  ****************************/
  ...
  for (i=0;i<15;i++)
    {
      num = num + ((&UART0_Format.Header)[i]);      //校验和函数
    }
  UART0_Format.Verify   = num;
  ****************************/
  UART0_Format.Verify   = 0x50;
...
{ 
  ...
  case SERIALAPP_CLUSTERID1:  //处理各个传感器节数据
    ...
    for (i=0;i<15;i++)
    {
      num = num + ((uint8*)(receiveData))[i];//校验和函数
    }
...
{  
...
#endif
  {
    ...
    if(SerialApp_TxLen > 0)
    {
      ...
      if((p->Header==0x40)&&(p->Verify==num))//包头包尾校验
      {
