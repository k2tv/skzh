...
uint8 Ctr5573VoltageOutput(uint16 Voltage,uint8 Port);
...
static uint8  RcvByte()
{
  ...
  uint8 BitCnt;
  IO_DIR_PORT_PIN(1, 0, IO_OUT);    //设置P1.0为输出
 // IO_DIR_PORT_PIN(1, 1, IO_OUT);    //设置P1.1为输出 change by wu 2011.3.1
  retc=0; 
 // SDA=1;             /*置数据线为输入方式*/
  IO_DIR_PORT_PIN(1, 1, IO_IN);
...
注    意：使用前必须已结束总线。
********************************************************************/
//static uint8 IRcvStr(uint8 sla,uint8 suba,uint8 *s,uint8 no)
//{
 //  uint8 i;
