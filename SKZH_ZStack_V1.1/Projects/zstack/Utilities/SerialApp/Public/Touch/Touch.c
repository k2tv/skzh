...
{ 
  ...
  if ( events & TOUCH_READ_EVT )  //查看光敏电阻状态
  {
    TouchStatusOld = TouchStatusNow;  //保存当前状态
    if(TOUCH_GPIO == HIGH)
    {
      TouchStatusNow = TOUCH_NO;
    }
    else if(TOUCH_GPIO == LOW)
    {
      TouchStatusNow = TOUCH_YES;
    }
    
    if(TouchStatusNow != TouchStatusOld) //如果当前状态和上一个状态发生变化，发送消息给网关
    {
      UART0_Format.Command = 0x01;
      if(TouchStatusNow == TOUCH_YES)
      {
        UART0_Format.Data[0] = 0x01;
        UART0_Format.Data[1] = 0x00;
      }
      else if(TouchStatusNow == TOUCH_NO)
      {
        UART0_Format.Data[0] = 0x00;
        UART0_Format.Data[1] = 0x00;       
      }
