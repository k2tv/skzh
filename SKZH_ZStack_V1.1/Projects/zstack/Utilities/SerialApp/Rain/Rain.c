...
static uint8 RainStatusOld;
static uint8 RainStatusNow;
/*******************************************************************************************
 *                                   LOCAL FUNCTIONS
 *******************************************************************************************/
...
*入口参数 ：无                                
*返 回 值 ：无                                                                                
*说    明 ：把P01设置为通用IO,输入模式                                                            
*****************************************************************************************/

static void PhotoInit(void) //P00  AD   P01   开关
{
  P0SEL &= ~0x02;  //P01设置为通用IO
  P0DIR &= ~0x02;  //将P01设置为输入模式 
  if(Rain_GPIO == HIGH)
    RainStatusNow = Rain_NO;
  else
    RainStatusNow = Rain_YES;
}
/*****************************************************************************************
*函数名称 ：串口初始化函数					                          
...
{ 
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SerialApp_TaskID )) )
    {
      ...
      case ZDO_STATE_CHANGE:
          ...
          if(SampleApp_NwkState == DEV_END_DEVICE) //判定当前设备类型
          {
            ...
            osal_set_event(SerialApp_TaskID, Rain_READ_EVT); //启动雨滴检测
          }
        break;
      default:
        break;
  ...
  if ( events & Rain_READ_EVT )  //查看雨滴模块状态
  {
    RainStatusOld = RainStatusNow;  //保存当前状态
    if(Rain_GPIO == HIGH)
    {
      RainStatusNow = Rain_NO;
    }
    else if(Rain_GPIO == LOW)
    {
      RainStatusNow = Rain_YES;
    }
    
    if(RainStatusNow != RainStatusOld) //如果当前状态和上一个状态发生变化，发送消息给网关
    {
      UART0_Format1.Command = 0x01;
      if(RainStatusNow == Rain_YES)
      {
        UART0_Format1.Data[0] = 0x01;
        
      }
      else if(RainStatusNow == Rain_NO)
      {
    ...
    osal_start_timerEx(SerialApp_TaskID, Rain_READ_EVT, 200);
    
    return ( events ^ Rain_READ_EVT );
  }
