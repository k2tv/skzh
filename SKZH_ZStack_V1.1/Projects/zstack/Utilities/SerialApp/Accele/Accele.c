...
*修改日期 ：2014.5.12  　　　　　　　　　　　　　　                                      *
*版 本 号 ：V1.0　　　　　　　　　　　　　　　　                                          *
*描    述 ：三轴加速度模块的终端节点程序，把测得的XYZ轴上的电压值打包发送给协调器                *
*                                                                                         *
******************************************************************************************/
...
UINT16 SerialApp_ProcessEvent( uint8 task_id, UINT16 events )
{
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SerialApp_TaskID )) )
    {
      ...
      case ZDO_STATE_CHANGE:
          ...
          if(SampleApp_NwkState == DEV_END_DEVICE) //判定当前设备类型
          {
            ...
            osal_set_event(SerialApp_TaskID, ACCELE_READ_EVT); //启动加速度检测
            LED(1,ON);
          }
        break;
      default:
  ...
  if ( events & ACCELE_READ_EVT )  //取ADLX335的AD采样值，步进：0.00293
  {
  ...
  //  uint16 AcceleResult;
    ...
    UART0_Format1.Data[1]=0x00;
    UART0_Format1.Data[2]=0x01;
    UART0_Format1.Data[3]=0x00;
    UART0_Format1.Data[4]=0x01;
    UART0_Format1.Data[5]=0x01;*/   //测试数据包 2014.5.14
   /* AcceleResult = HalAdcRead (HAL_ADC_CHANNEL_5 ,HAL_ADC_RESOLUTION_12); //max = 2048 (x-1024)*0.004878
  
    UART0_Format1.Data[0] =AcceleResult>>8;
    UART0_Format1.Data[1] = AcceleResult;
 
    AcceleResult = HalAdcRead (HAL_ADC_CHANNEL_6 ,HAL_ADC_RESOLUTION_12);
   
    UART0_Format1.Data[2] = AcceleResult>>8;
    UART0_Format1.Data[3] = AcceleResult;
    AcceleResult = HalAdcRead (HAL_ADC_CHANNEL_7 ,HAL_ADC_RESOLUTION_12);
    UART0_Format1.Data[4] = AcceleResult>>8;
    UART0_Format1.Data[5] = AcceleResult;*/
    
    //通道5
    ADC_ENABLE_CHANNEL(ADC_AIN5);                          // 使能AIN5为ADC输入
    
    ADC_SINGLE_CONVERSION(ADC_REF_AVDD | ADC_12_BIT | ADC_AIN5);

    ADC_SAMPLE_SINGLE();                                   // 启动一个单一转换

    while(!ADC_SAMPLE_READY());                            // 等待转换完成

    ADC_ENABLE_CHANNEL(ADC_AIN5);                          // 禁止AIN5

      
    UART0_Format1.Data[0] =ADCH;
    UART0_Format1.Data[1] = ADCL;
    
    //通道6
    ADC_ENABLE_CHANNEL(ADC_AIN6);                          // 使能AIN6为ADC输入
    
    ADC_SINGLE_CONVERSION(ADC_REF_AVDD | ADC_12_BIT | ADC_AIN6);

    ADC_SAMPLE_SINGLE();                                   // 启动一个单一转换

    while(!ADC_SAMPLE_READY());                            // 等待转换完成

    ADC_ENABLE_CHANNEL(ADC_AIN6);                          // 禁止AIN6
    
    UART0_Format1.Data[2] =ADCH;
    UART0_Format1.Data[3] =ADCL;
    
    //通道4
    ADC_ENABLE_CHANNEL(ADC_AIN4);                          // 使能AIN4为ADC输入
    
    ADC_SINGLE_CONVERSION(ADC_REF_AVDD | ADC_12_BIT | ADC_AIN4);

    ADC_SAMPLE_SINGLE();                                   // 启动一个单一转换

    while(!ADC_SAMPLE_READY());                            // 等待转换完成

    ADC_ENABLE_CHANNEL(ADC_AIN4);                          // 禁止AIN4
    
    UART0_Format1.Data[4] =ADCH;
    UART0_Format1.Data[5] =ADCL;
    
    osal_set_event(SerialApp_TaskID, SERIALAPP_SEND_EVT);
    osal_start_timerEx(SerialApp_TaskID, ACCELE_READ_EVT,500);
    return ( events ^ ACCELE_READ_EVT );
  }
